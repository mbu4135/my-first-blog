...
def carts():
    if 'uid' in session:
        ...
        cur.execute('CREATE TABLE uorders SELECT uid,ofname,pid,quantity,dstatus,odate,ddate FROM orders WHERE uid=%s', (uid,))
        cur.execute('ALTER TABLE uorders ADD COLUMN id INT NOT NULL AUTO_INCREMENT PRIMARY KEY')
        cur.execute('DROP TABLE IF EXISTS uproducts')
        cur.execute('CREATE TABLE uproducts SELECT pid,pName,price,description,available,category,item,pCode,picture,date FROM products')
        cur.execute('CREATE OR REPLACE VIEW cart AS SELECT id,pid,pName,price,quantity,uid FROM uproducts INNER JOIN uorders USING (pid)')
        cur.execute('SELECT * FROM cart')
        result = cur.fetchall()
        cur.execute('SELECT id,pid,pName,price,quantity,uid,(price*quantity) AS total_price FROM cart')
        row1 = cur.fetchall()
        cur.execute('SELECT id,pid,pName,price,quantity,uid,(price*quantity) AS total_price FROM cart WHERE id=1')
        row2 = cur.fetchall()
        cur.execute('SELECT id,pid,pName,price,quantity,uid,(price*quantity) AS total_price FROM cart WHERE id=2')
        row3 = cur.fetchall()
        cur.execute('SELECT id,pid,pName,price,quantity,uid,(price*quantity) AS total_price FROM cart WHERE id=3')
        row4 = cur.fetchall()
        cur.execute('SELECT id,pid,pName,price,quantity,uid,(price*quantity) AS total_price FROM cart WHERE id=4')
        row5 = cur.fetchall()
        cur.execute('SELECT id,pid,pName,price,quantity,uid,(price*quantity) AS total_price FROM cart WHERE id=5')
        row6 = cur.fetchall()
        cur.execute('SELECT id,pid,pName,price,quantity,uid,(price*quantity) AS total_price FROM cart WHERE id=6')
        row7 = cur.fetchall()
        cur.execute('SELECT id,pid,pName,price,quantity,uid,(price*quantity) AS total_price FROM cart WHERE id=7')
        row8 = cur.fetchall()
        cur.execute('SELECT id,pid,pName,price,quantity,uid,(price*quantity) AS total_price FROM cart WHERE id=8')
        row9 = cur.fetchall()
        cur.execute('SELECT id,pid,pName,price,quantity,uid,(price*quantity) AS total_price FROM cart WHERE id=9')
        row10 = cur.fetchall()
        cur.execute('SELECT id,pid,pName,price,quantity,uid,(price*quantity) AS total_price FROM cart WHERE id=10')
        row11 = cur.fetchall()
        cur.execute('SELECT id,pid,pName,price,quantity,uid,(price*quantity) AS total_price FROM cart WHERE id=11')
        cur.close()
        if request.method == 'POST' and form.validate():
            ...
            if num == '1':
                ...
                curs.execute('INSERT INTO uorders (uid,pid,quantity) VALUES (%s,%s,%s)', (uid,16,1,))
            curs.execute('DELETE FROM orders WHERE uid=%s', (uid,))
            curs.execute('ALTER TABLE orders DROP COLUMN id')
            curs.execute('INSERT orders SELECT uid,ofname,pid,quantity,dstatus,odate,ddate FROM uorders')
        ...
        cur.execute('DROP TABLE IF EXISTS uorders')
        cur.execute('CREATE TABLE uorders SELECT uid,ofname,pid,quantity,dstatus,odate,ddate FROM orders WHERE ofname=%s', (ofname,))
        cur.execute('ALTER TABLE uorders ADD COLUMN id INT NOT NULL AUTO_INCREMENT PRIMARY KEY')
        cur.execute('DROP TABLE IF EXISTS uproducts')
        cur.execute('CREATE TABLE uproducts SELECT pid,pName,price,description,available,category,item,pCode,picture,date FROM products')
        cur.execute('CREATE OR REPLACE VIEW cart AS SELECT id,pid,pName,price,quantity,uid,ofname FROM uproducts INNER JOIN uorders USING (pid)')
        cur.execute('SELECT * FROM cart')
        result = cur.fetchall()
        cur.execute('SELECT id,pid,pName,price,quantity,uid,ofname,(price*quantity) AS total_price FROM cart')
        row1 = cur.fetchall()
        cur.execute('SELECT id,pid,pName,price,quantity,uid,ofname,(price*quantity) AS total_price FROM cart WHERE id=1')
        row2 = cur.fetchall()
        cur.execute('SELECT id,pid,pName,price,quantity,uid,ofname,(price*quantity) AS total_price FROM cart WHERE id=2')
        row3 = cur.fetchall()
        cur.execute('SELECT id,pid,pName,price,quantity,uid,ofname,(price*quantity) AS total_price FROM cart WHERE id=3')
        row4 = cur.fetchall()
        cur.execute('SELECT id,pid,pName,price,quantity,uid,ofname,(price*quantity) AS total_price FROM cart WHERE id=4')
        row5 = cur.fetchall()
        cur.execute('SELECT id,pid,pName,price,quantity,uid,ofname,(price*quantity) AS total_price FROM cart WHERE id=5')
        row6 = cur.fetchall()
        cur.execute('SELECT id,pid,pName,price,quantity,uid,ofname,(price*quantity) AS total_price FROM cart WHERE id=6')
        row7 = cur.fetchall()
        cur.execute('SELECT id,pid,pName,price,quantity,uid,ofname,(price*quantity) AS total_price FROM cart WHERE id=7')
        row8 = cur.fetchall()
        cur.execute('SELECT id,pid,pName,price,quantity,uid,ofname,(price*quantity) AS total_price FROM cart WHERE id=8')
        row9 = cur.fetchall()
        cur.execute('SELECT id,pid,pName,price,quantity,uid,ofname,(price*quantity) AS total_price FROM cart WHERE id=9')
        row10 = cur.fetchall()
        cur.execute('SELECT id,pid,pName,price,quantity,uid,ofname,(price*quantity) AS total_price FROM cart WHERE id=10')
        row11 = cur.fetchall()
        cur.execute('SELECT id,pid,pName,price,quantity,uid,ofname,(price*quantity) AS total_price FROM cart WHERE id=11')
        cur.close()
        if request.method == 'POST' and form.validate():
            num = request.form['num']
            delete = form.delete.data
            ...
            if num == '1':
                ...
                curs.execute('INSERT INTO uorders (ofname,pid,quantity) VALUES (%s,%s,%s)', (ofname,16,1,))
...
class BuyForm(Form):
    ...
    memo = TextAreaField('', [validators.length(min=4, max=100)],render_kw={'autofocus': True})
...
def buy_product():
    ...
    if 'uid' in session:
        ...
        uid = session['uid']
        cur = mysql.connection.cursor()
        cur.execute('SELECT id,pid,pName,price,quantity,uid,(price*quantity) AS total_price FROM cart WHERE uid=%s', (uid,))
        product = cur.fetchall()
        cur.execute('SELECT SUM(price*quantity) AS total_price FROM cart WHERE uid=%s', (uid,))
        total = cur.fetchall()
        cur.close()
        curs = mysql.connection.cursor()
        ...
        if request.method == 'POST' and form.validate():
            ...
            curso.execute('INSERT buy_product SELECT id,uid,pid,quantity,dstatus,odate,ddate FROM uorders')
            curso.execute('INSERT INTO buy_product (rmobile1,rmobile2,rmobile3,addnum,oplace,oplaced,memo) VALUES (%s,%s,%s,%s,%s,%s,%s)', (rmobile1,rmobile2,rmobile3,addnum,oplace,oplaced,memo,))
        ...
        ofname = session['ofname']
        cur = mysql.connection.cursor()
        cur.execute('SELECT id,pid,pName,price,quantity,uid,(price*quantity) AS total_price FROM cart WHERE ofname=%s', (ofname,))
        product = cur.fetchall()
        cur.execute('SELECT SUM(price*quantity) AS total_price FROM cart WHERE ofname=%s', (ofname,))
        total = cur.fetchall()
        cur.close()
        if request.method == 'POST' and form.validate():
            curso = mysql.connection.cursor()
...
def a1():
    ...
    if request.method == 'POST' and form.validate():
        ...
        if 'uid' in session:
            ...
            curs.execute("INSERT INTO orders(uid, ofname, pid, quantity, ddate) VALUES(%s, %s, %s, %s, %s)", (uid, ofname, pid, quantity, now_time,))
        else:
            ofname = request.form['ofname']
            curs.execute("INSERT INTO orders(pid, ofname, quantity, ddate) VALUES(%s, %s, %s, %s)", (pid, ofname, quantity, now_time,))
